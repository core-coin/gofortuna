...
const (
	maxBlocks    int = 65536
	newKeyBlocks int = 2 // len(rngKey) / twofish.BlocksSize
)
...
func (g *Generator) Read(p []byte) (int, error) {
	...
	pl := len(p)
	k := pl / twofish.BlockSize
	if (pl % twofish.BlockSize) != 0 {
		k++
	}
	gr := k / maxBlocks
	if (k % maxBlocks) != 0 {
	...
	for i := 0; i < gr; i++ {
		...
		if err != nil {
			return i * twofish.BlockSize, err
		}
		if i < (gr - 1) {
			pp = pp[blks*twofish.BlockSize:]
		}
	}
	return pl, nil
}
